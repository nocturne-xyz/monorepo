...
describe(
  ...
  async () => {
    ...
    beforeEach(async () => {
      ...
      ({ teardown, aliceEoa, provider, depositManager, handler, provider } = await setupTestDeployment({
        include: {
          subgraph: true,
    ...
    it("pulls only data for the given block range", async () => {
      ...
      {
        const diffs =  syncAdapter.iterStateDiffs(0, { endBlock: firstRangeEndBlockExpected });
      ...
      {
        const diffs =  syncAdapter.iterStateDiffs(firstRangeEndBlock, { endBlock: secondRangeEndBlockExpected });
        for await (const { notes, blockNumber } of diffs.iter) {
          notes.forEach(note => {
            expect(NoteTrait.isEncryptedNote(note)).to.be.false;
            const commitment = NoteTrait.toCommitment(note as IncludedNote);
        ...
        expect(notesFirstRange.size).to.equal(4);
      }
    }); 
  }
);