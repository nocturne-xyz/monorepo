...
contract OffchainMerkleTree is IOffchainMerkleTree {
    ...
    function root() external override view returns (uint256) {
		return _root;
	}


	// returns the current number of leaves in the tree
    function count() external override view returns (uint128) {
		return _count;
	}

    function totalCount() external override view returns (uint128) {
		return _count + batchLen + uint128(Utils.BATCH_SIZE) * uint128(accumulatorQueue.length());
    }
    ...
    function applySubtreeUpdate(
		...
		require(!accumulatorQueue.isEmpty(), "accumulatorQueue is empty - no batches to apply");

        uint256 accumulatorHash = accumulatorQueue.peek();
        (uint256 hi, uint256 lo) = Utils.uint256ToFieldElemLimbs(accumulatorHash);
