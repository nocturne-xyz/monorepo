...
library Utils {
	...
	uint256 public constant EMPTY_TREE_ROOT = 21443572485391568159800782191812935835534334817699172242223315142338162256601;
	...
	function splitUint256ToLimbs(uint256 n, uint256 lowerBits) pure internal returns (uint256, uint256) {
	...
	function encodePathAndHash(uint128 subtreeIdx, uint256 accumulatorHashHi) pure internal returns (uint256) {
		require(subtreeIdx % BATCH_SIZE == 0, "subtreeIdx not multiple of BATCH_SIZE");
        uint256 encodedPathAndHash = uint256(subtreeIdx) >> BATCH_SUBTREE_DEPTH;
		encodedPathAndHash |= accumulatorHashHi << (DEPTH - BATCH_SUBTREE_DEPTH);
