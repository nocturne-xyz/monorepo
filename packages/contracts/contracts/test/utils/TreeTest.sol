...
library TreeTestLib {
	uint256 public constant EMPTY_SUBTREE_ROOT = 3607627140608796879659380071776844901612302623152076817094415224584923813162;
	...
	function computeSubtreeRoot(TreeTest storage self, uint256[] memory batch) internal returns (uint256) {
	...
	function computeInitialRoot(TreeTest storage self, uint256[] memory batch) internal returns (uint256[] memory) {
		uint256 subtreeRoot = computeSubtreeRoot(self, batch);
		uint256 zero = EMPTY_SUBTREE_ROOT;

		uint256[] memory path = new uint256[](Utils.DEPTH - Utils.BATCH_SUBTREE_DEPTH + 1);
		path[0] = subtreeRoot;
	...
	function computeNewRoot(TreeTest storage self, uint256[] memory batch, uint256[] memory path, uint256 idx) internal returns (uint256[] memory) {
		uint256 subtreeRoot = computeSubtreeRoot(self, batch);
		uint256 subtreeIdx = idx >> Utils.BATCH_SUBTREE_DEPTH;
		uint256 zero = EMPTY_SUBTREE_ROOT;

		uint256[] memory newPath = new uint256[](Utils.DEPTH - Utils.BATCH_SUBTREE_DEPTH + 1);
	...
	function computeNoteCommitment(TreeTest storage self, IWallet.Note memory note) internal returns (uint256) {
		uint256 addrHash = self.hasherT3.hash([note.ownerH1, note.ownerH2]);
		return self.hasherT6.hash([
			addrHash,
			note.nonce,
			note.asset,
			note.id,
