...
export class NocturneSdk implements NocturneSdkApi {
  ...
  async getAllDeposits(): Promise<DepositHandle[]> {
    const spender = (await this.getWindowSigner()).getAddress();
    ...
    return data.depositRequests.map(toDepositRequestWithMetadata).map((dr) => {
      const depositRequestHash = hashDepositRequest(dr);
      return {
        depositRequestHash,
        request: dr,
  ...
  async fetchDepositRequestStatus(
    ...
    return {
      estimatedWaitSeconds: depositStatusResponse.estimatedWaitSeconds,
      substatus: depositStatusResponse.status,
    };
  }
  ...
  private async formInitiateDepositResult(
    ...
    const depositRequest: DepositRequestWithMetadata = {
