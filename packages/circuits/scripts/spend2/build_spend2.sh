...
# # start=`date +%s`
# # node "$BUILD_DIR"/"$CIRCUIT_NAME"_js/generate_witness.js "$BUILD_DIR"/"$CIRCUIT_NAME"_js/"$CIRCUIT_NAME".wasm input_spend2.json "$BUILD_DIR"/witness.wtns
# # end=`date +%s`
# # echo "DONE ($((end-start))s)"
...
echo "$OUTPUT_DIR"/"$CIRCUIT_NAME"

echo "****EXPORTING SOLIDITY SMART CONTRACT****"
start=`date +%s`
npx snarkjs zkey export solidityverifier "$OUTPUT_DIR"/"$CIRCUIT_NAME".zkey "$CONTRACTS_DIR/Spend2Verifier.sol"
end=`date +%s`
echo "DONE ($((end-start))s)"