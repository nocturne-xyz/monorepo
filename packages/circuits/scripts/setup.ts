...
async function setup(circuitName: string, log2_size: number) {
	await asyncExec(`yarn snarkjs powersoftau prepare phase2 ${PTAU_DIR}/pot${log2_size}_0001.ptau ${SETUP_DIR}/pot${log2_size}${circuitName}_final.ptau -v`)
	...
	await asyncExec(`yarn snarkjs zkey export verificationkey ${SETUP_DIR}/${circuitName}_0001.zkey ${SETUP_DIR}/${circuitName}proof_key.json`)
}
