...
export async function fetchLatestIndexedBlock(
  ...
  if (error || !data) {
    throw new Error(error?.message ?? "LatestIndexedBlock query failed");
  }

  const sdkEventBlock = data.sdk_event_aggregate.aggregate?.max?.block;
  const subtreeCommitBlock =
    data.subtree_commit_aggregate.aggregate?.max?.block;

  return maxNullish(sdkEventBlock, subtreeCommitBlock)!;
}
