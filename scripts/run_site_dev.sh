...
read VAULT_CONTRACT_ADDR < <(sed -nr 's/deploying "Vault_Proxy" \(tx: 0x[0-9a-fA-F]+\)\.\.\.: deployed at (0x[0-9a-fA-F]+) with [0-9]+ gas/\1/p' $LOG_DIR/hh-node)
...
echo "Vault contract address: $VAULT_CONTRACT_ADDR"
...
sed -i '' -r -e "s/const VAULT_ADDRESS = \"0x[0-9a-faA-F]+\";/const VAULT_ADDRESS = \"$VAULT_CONTRACT_ADDR\";/g" $SNAP_INDEX_TS
