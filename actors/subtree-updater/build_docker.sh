...
if [ ! -z "$IS_MOCK" ]; then
    ...
    docker build -t nocturnelabs/mock-subtree-updater:$COMMIT_HASH -f ./packages/subtree-updater/Mock.Dockerfile .
    exit 0
fi
...
if [[ $(uname -m) == 'arm64' ]]; then
    ...
    docker buildx build --platform linux/amd64 -t nocturnelabs/subtree-updater:$COMMIT_HASH -f ./packages/subtree-updater/Dockerfile .
else
    echo 
    docker build -t rapidsnark ./rapidsnark
    docker build -t nocturnelabs/subtree-updater:$COMMIT_HASH -f ./packages/subtree-updater/Dockerfile .
fi
