...
import { merkleIndexFromRedisStreamId, merkleIndexToRedisStreamId } from "./utils";
...
export class InsertionWriter {
  ...
  logger: Logger;
    ...
    const newInsertionBatches = this
			.adapter
			.iterInsertions(logTipMerkleIndex ?? 0, { throttleMs: queryThrottleMs });
		...
		const runProm = (async () => {
			...
			for await (const insertions of newInsertionBatches.iter) {
				...
				await this.insertionLog.push(
					insertions.map(insertion => ({ inner: insertion, id: merkleIndexToRedisStreamId(insertion.merkleIndex) }))
				);
				this.logger.info(`pushed batch of ${insertions.length} into insertion log`, meta);
			}
		})();
		...
		const teardown = async () => {
			...
			this.logger.debug("closed insertion iterator. waiting for main loop to terminate...");
			await runProm;
